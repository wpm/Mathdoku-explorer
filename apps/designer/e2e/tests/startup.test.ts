...
test.describe('startup', () => {
  ...
  test('get_puzzle returning null shows Size Modal instead of grid', async ({ page }) => {
  ...
  test('startup Size Modal has no Cancel button and Escape does not dismiss it', async ({ page }) => {
    await installTauriStubs(page, null);
    await gotoApp(page);
    await expect(page.locator('p').filter({ hasText: 'New puzzle' })).toBeVisible();
